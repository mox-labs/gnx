...
*This is the alignment document — what the whole ecosystem is made of, semantically, and who owns which piece. Converged in discourse 2026-07-06 (yzavyas × Fable) after five deep-read verifications of the shipped artifacts. Where a claim is one party's synthesis rather than corpus doctrine, it is marked.*

## The sprawl, and what it actually is

The ecosystem's history looks scattered: matrix has components-with-contracts over a board; ix has type_url→factory registries and a configurable DAG; recon declares catalog+collectors bound by name; geist-edge runs processors from a typed registry; the hud designed panels-with-manifests; luminex sketched a Flow. Three things carry the name "slick," five carry "geist," two carry "Construct."
...
The clean-room job is therefore **extraction and canonization, not invention**. Anyone about to stand up a sixth implementation — a new runtime, a new registry, a new board — needs that distinction before they start: the five primitives below are what to extend, not what to re-derive under a new name.
...
|---|---|---|
| **the attestation** | `TypedStruct` — a `type_url` plus an opaque value; the atom of data *(renamed from "the mark" — D19)* | slick `TypedStruct`, matrix component outputs, xDS `Any`/`TypedStruct` |
...
| **the board** | an append-only ledger of provenance-wrapped attestations with a single mediated writer; the atom of coordination | matrix `Construct`, recon's timestamped run archives, luminex's radix face, the hud's "the board mediates collaboration" |
...
2. **Coordination by vocabulary, never co-location.** Components share the grammar, not each other's interiors. A producer emits an attestation of a known type_url; any consumer that understands it can act, with zero access to the producer's inside.
...
4. **Config is data, never code — defined or opaque.** A member's config validates against a schema when one exists and rides as an opaque `TypedStruct` until then (ix's Pydantic-at-the-factory is the shipped proof of the defined half).
...
7. **Humans are exogenous.** Steering happens as appends to the board through the single mediated writer, in epochs between runs — never as a step compiled into the execution cycle itself. The human is in the oversight loop, never a DAG node. No source in the corpus models the human as a scheduled member.
...
9. **Mint once, alias forever.** Serialized names are permanent; lineage and supersession live in `relations`, never in identities. Applies to type_urls, GEP numbers, enum values, and project names alike. The moment this rule bites is the one where renaming looks obviously correct — a component author fixing a name reaches for `relations`, not a rename.
...
- **Canonical home** — slick (primitives + validity), gnx (the ecology).
