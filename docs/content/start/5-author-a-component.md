...
**Build a capability once; make it composable by any agent that speaks the grammar — Claude Code today — in any project that declares it needs it.**

**This loop needs the gnx CLI — planned, not yet shipped.** The shape below is the designed authoring path. Nothing here is runnable today.
...
A component earns its place in the catalog by being useful on its own.

Before authoring: if someone installs only this component — not the others it relates to, not the full catalog — does it still do something? A Skill that needs context from five other Skills to say anything is not standalone. A Capability that only makes sense inside one Flow is not standalone — it's a fragment of that Flow.
...
`gnx component init [kind]` will scaffold three things:
...
The scaffold is kind-aware. A `Capability` template differs from a `Skill` template — different required fields, different port shapes, different curation requirements.
...
The grammar boundary is where correctness is enforced — not at runtime, not by convention, not by code review. What passes validate is what the registry will accept.
...
A Capability without an embedded skill is not agent-operable. Claude Code reads a skill to drive a tool; it cannot read a man page. The curation bar enforces this — not optional for Capabilities, not waivable post-registration.
...
Both generated manifests come from one source. Dual-manifest version drift is structurally impossible when one write generates both.

Projection happens at authoring time because Claude Code's install mechanics leave no alternative. When CC installs a plugin, it copies only the plugin subdirectory into a versioned cache. No install-time build hook exists. What lands in the plugin subdir at clone time is what CC sees. The plugin dir must be self-contained; path traversal out of it is banned.
