...
name: optimizer
description: >-
  Fix issues identified by the evaluator agent. Use this agent when: evaluator returned "NEEDS WORK",
  have a list of issues to fix, want to improve quality score. Systematic fixes, not rewrites. Typical
  triggers: plugin failed quality evaluation; plugin needs to reach quality threshold. See "When to
...
- **Plugin failed quality evaluation.** User: "Fix the issues the evaluator found" Assistant: "I'll use the optimizer agent to address each issue systematically." *Optimizer takes evaluator output and applies targeted fixes.*
- **Plugin needs to reach quality threshold.** User: "Optimize this plugin to pass quality gates" Assistant: "I'll use the optimizer agent to fix issues until it passes." *Optimizer iterates until all gates pass minimum threshold.*
