...
def substitute(value: Any, variables: dict[str, str]) -> Any:
    ...
    if isinstance(value, str):
        ...
        def replacer(match: re.Match[str]) -> str:
            key = match.group(1)
            # `.get` on a dict[str, str] with a str default returns str; mypy needs
            # the annotation because `variables` arrives untyped from config.
    ...
    for entry in config.collectors:
        ...
        if entry.source:
            if entry.source not in catalog:
                ...
                continue
            # Annotated explicitly: the else-branch below legitimately yields None for a
            # collector with no catalog source, so the inferred non-optional type is wrong.
            targets: list[tuple[SourceEntry | None, str]] = [
                (catalog[entry.source], entry.name)
            ]
        elif config.catalog:
