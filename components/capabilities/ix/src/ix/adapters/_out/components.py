...
Each is a Matrix Component: name, requires, provides, async run(construct) → TypedStruct.
...
class TrialNode:
    ...
    name = "trial"
    requires: frozenset[str] = frozenset({"probe.stimulus", "subject.config"})
    provides = "trial.observation"
...
class SensorNode:
    ...
    name = "sensor"
    requires: frozenset[str] = frozenset({"trial.observation"})
    provides = "sensor.reading"
