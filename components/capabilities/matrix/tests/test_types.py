...
class TestArtifact:
    ...
    def test_frozen(self):
        """Mutating a frozen Artifact raises pydantic's ValidationError.

        Named specifically, not `Exception`: a blind assert here passes on ANY error —
        including an AttributeError from a typo in this test — so it would keep passing
        even if Artifact stopped being frozen and the test itself broke instead.
        """
        a = Artifact.create(type_url="test.v1/data", producer="probe", data="hello")
        with pytest.raises(ValidationError):
            a.data = "mutate"
        assert a.data == "hello", "the frozen instance must be unchanged"
...
class TestContractError:
    ...
    def test_message(self):
        err = ContractError("provides='X' but returned type_url='Y'")
        assert "X" in str(err)
        assert "Y" in str(err)
