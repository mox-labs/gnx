...
class ReadingComponent:
    ...
    async def run(self, construct: Construct) -> TypedStruct:
        ...
        return TypedStruct(type_url=self.provides, value=f"read:{upstream}")
...
class MismatchComponent:
    ...
    def __init__(self, name: str, provides: str, actual_type_url: str):
        self.name = name
...
class TestOrchestrator:
    ...
    async def test_contract_error_on_type_url_mismatch(self):
        """Orchestrator raises ContractError when type_url != provides."""
        bad = MismatchComponent(
            name="liar",
            provides="declared.type",
            actual_type_url="actual.type",
        )

        orch = Orchestrator([bad])
        with pytest.raises(ContractError, match="declared provides='declared.type'"):
            await orch.run()
