...
See also: the `mrwolf` agent, which operationalises this reframing. (An earlier revision linked a
`hooks/PostToolUse/detect-debugging-loop.sh`; no such hook was ever shipped, so the link pointed at
nothing.)
