...
description: This skill should be used when the user asks to "harden this intent", "what do they actually
  mean", "sharpen this", "is this ready to build", or "what's the real ask here" — or when a vague
  request, an unsettled decision, or a load-bearing underspecified requirement must become something
  the catalog can act on.
---
...
Intent is not extracted from an utterance — it is **hardened** through structured discourse against a substrate that pre-conditions both interlocutors. The ask runs through an adhikaraṇa-structured exchange (locus → doubt → steelmanned opposition → resolution → what-changes-downstream), terminates at triguṇa-completeness (the interpreter's state, not the text's), and reports how far the intent can honestly harden per the gnx gradient (structural / behavioral / semantic).

Pairs with `rational-inquiry` — the validity gate run inside the resolution step — and is the discipline the `bodhi` agent operationalises; it runs at bodhi's envision step.
