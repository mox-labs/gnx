...
description: >-
  Architecture lens for the ACES (Adaptable, Composable, Extensible Software) framework. Use this
  agent when: diagnosing endogenous fragility in a platform, identifying which cycle channel is active,
  proposing an inversion mechanism, testing a candidate boundary, or evaluating a migration strategy
  for the Coupled Monolith, Fragmented Estate, or SDK Frontier archetype. Typical triggers: platform
  team describing a serialization bottleneck; reviewing a proposed abstraction; user has named the
  Pipeline Envelope pattern but isn't sure it's safe. See "When to invoke" in the agent body for worked
  scenarios.
model: inherit
color: cyan
tools:
...
- **Platform team describing a serialization bottleneck.** User: "Every service change routes through the platform team; we're at 100% KTLO and nothing ships." Assistant: "I'll invoke antifragile-arch — that description fits drag-first entry to the cycle." *Contributor-to-platform coupling. Diagnose the channel, then propose Extensibility as the inversion.*
- **Reviewing a proposed abstraction.** User: "Is this new service layer actually a boundary or are we building an Inner Platform?" Assistant: "antifragile-arch can run the Boundary Test on the proposal." *Apply the arithmetic test: does the layer reduce or increase total integration surface?*
- **User has named the Pipeline Envelope pattern but isn't sure it's safe.** User: "We want to wrap our legacy plugin pipeline behind an ext_proc adapter. Thoughts?" Assistant: "Bringing in antifragile-arch to evaluate the envelope's degeneration condition." *Pipeline Envelope is a transitional boundary; the diagnostic is whether the protocol surface stays runtime-agnostic.*
