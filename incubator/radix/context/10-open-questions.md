...
- **OD-10 — ecosystem shape.** [PARTIALLY RULED 2026-08-02 — the rest stays OPEN] **L2 processor vs L3 component**; manifest convention; mint `type_urls`; **gnx placement.**
  - **Ruled:** the `kind:` field no longer waits on OD-10. The core kind set is four — Skill, Agent, Flow, Capability — and a processor is a *Capability that runs in the data plane*, not a kind of its own. radix is `kind: Capability`, filed at `components/capabilities/radix/`. This revises D25 (which had adopted Processor and View into the core set).
  - **Still open:** whether produces/consumes topology is first-class on the manifest; the L2/L3 shape itself; how a Capability declares *which* shape it is (processor / api / …) — there is no sub-type mechanism today. Do not presume the L2/L3 shape.
