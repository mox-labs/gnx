...
- The `crafting` skill in the **ci-scaffolds** plugin - Methodology definition. Referenced by
  name rather than by path: it ships in a different plugin, and plugin install copies only one
  plugin's own subdirectory, so a relative link there resolves in the source repo and nowhere else.
